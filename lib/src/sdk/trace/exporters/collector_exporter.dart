...
class CollectorExporter implements sdk.SpanExporter {
  ...
  Future<void> _send(Uri uri, List<sdk.ReadOnlySpan> spans) async {
    const maxRetries = 3;
    var retries = 0;
    ...
    while (retries < maxRetries) {
      ...
      final delay = calculateJitteredDelay(
        retries++,
  ...
  Iterable<pb_trace.Span_Event> _spanEventsToProtobuf(
    ...
    return events.map(
      (event) => pb_trace.Span_Event(
        timeUnixNano: event.timestamp,
        name: event.name,
        attributes: event.attributes.map(
          (attribute) => pb_common.KeyValue(
            key: attribute.key,
  ...
  pb_trace.Span _spanToProtobuf(sdk.ReadOnlySpan span) {
    ...
    return pb_trace.Span(
      ...
      attributes: span.attributes.keys.map(
        (key) => pb_common.KeyValue(
          key: key,
          value: _attributeValueToProtobuf(span.attributes.get(key)!),
        ),
      ),
      droppedAttributesCount: span.attributes.length > 0
          ? span.droppedAttributes
          : null,
      events: _spanEventsToProtobuf(span.events),
      droppedEventsCount: span.events.isNotEmpty
